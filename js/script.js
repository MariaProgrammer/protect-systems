document.addEventListener('DOMContentLoaded', () => {
    ...
    dropdownParents.forEach(parent => {
        ...
        if (!dropdown) return; // Пропускаем, если у пункта нет выпадающего меню

        const leftItems = dropdown.querySelectorAll('.dropdown-menu__item');
        const submenus = dropdown.querySelectorAll('.submenu');

        // Функция для показа конкретного подменю
        const showSubmenu = (id) => {
            leftItems.forEach(item => item.classList.toggle('is-active', item.dataset.submenuId === id));
            submenus.forEach(submenu => submenu.classList.toggle('is-active', submenu.dataset.submenuId === id));
        };

        // Назначаем обработчики наведения на пункты левой колонки
        leftItems.forEach(item => {
            item.addEventListener('mouseenter', () => {
                showSubmenu(item.dataset.submenuId);
            });
        });

        // Логика при наведении на родительский пункт (например, "Services")
        parent.addEventListener('mouseenter', () => {
            ...
            // Открываем текущее меню и оверлей
            parent.classList.add('is-active');
            showOverlay();

            // Показываем первое подменю по умолчанию
            if (leftItems.length > 0 && !dropdown.querySelector('.dropdown-menu__item.is-active')) {
                showSubmenu(leftItems[0].dataset.submenuId);
            }
        });
    });

    // Логика закрытия меню при уходе курсора со всей навигации
    nav.addEventListener('mouseleave', () => {
        leaveTimeout = setTimeout(() => {
            ...
            hideOverlay();
        }, 300); // Небольшая задержка, чтобы можно было успеть навести на меню
    });
